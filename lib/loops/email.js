...
export async function sendBookingConfirmationEmail(bookingData) {
  try {
    ...
    if (!apiKey) {
      console.warn(
        "LOOPS_API_KEY or NEXT_LOOP_KEY not set, skipping email send"
      );
    ...
    if (
      ...
      return {
        success: false,
        error: "Invalid or missing customer email address",
      };
    }
    ...
    const addonsList =
      addons.length > 0
        ? addons.map(
            (addon) =>
              `• ${addon.name} (${addon.adult || 0} adults, ${
                addon.child || 0
              } children)`
          )
        : ["• No addons selected"];
    ...
    if (response.success) {
      console.log(
        `Booking confirmation email sent to ${customerEmail} for booking ${bookingId}`
      );
      return { success: true, messageId: response.id };
    } else {
      console.error(
        "Failed to send booking confirmation email. Response:",
...
export async function sendPackageApprovalEmail(bookingData) {
  try {
    ...
    if (response.success) {
      console.log(
        `Package approval email sent to ${customerEmail} for booking ${bookingId}`
      );
      return { success: true, messageId: response.id };
    } else {
      console.error(
        "Failed to send package approval email. Response:",
        JSON.stringify(response, null, 2)
      );
      return {
        success: false,
...
/**
 * Send admin notification email when a new booking requires approval
 * @param {Object} bookingData - Booking information
 * @param {string} bookingData.bookingId - Booking ID
 * @param {string} bookingData.packageName - Package/tour name
 * @param {string} bookingData.customerName - Customer full name
 * @param {string} bookingData.customerEmail - Customer email address
 * @param {string} bookingData.customerPhone - Customer phone number
 * @param {string} bookingData.travelDate - Travel date
 * @param {number} bookingData.adults - Number of adults
 * @param {number} bookingData.children - Number of children
 * @param {number} bookingData.totalAmount - Total amount
...
export async function sendAdminBookingNotificationEmail(bookingData) {
  try {
    ...
    // Admin email address
    const adminEmail = "urbanadventuretourism@gmail.com";
    ...
    // Create email template data with the specific template ID for admin notification
    const emailData = {
      email: adminEmail.trim(), // Admin email address
      transactionalId: "cmjsvt0fx19vg0ix39r3of6is", // Admin notification template ID
      dataVariables: {
        bookingId: bookingId || "N/A",
        packageName: packageName || "Tour Package",
    ...
    console.log(
      "Sending admin booking notification email to:",
      adminEmail.trim()
    );
    console.log("Email data prepared:", {
      email: emailData.email,
    ...
    if (response.success) {
      console.log(
        `Admin booking notification email sent to ${adminEmail} for booking ${bookingId}`
      );
      return { success: true, messageId: response.id };
    } else {
      console.error(
        "Failed to send admin booking notification email. Response:",
        JSON.stringify(response, null, 2)
      );
      return {
        success: false,
    ...
    console.error("Error details:", {
      ...
      statusCode: error.statusCode,
      json: error.json,
      transactionalId: "cmjsvt0fx19vg0ix39r3of6is",
    });
    return {
      success: false,
      error: error.message || "An error occurred while sending email",
...
export async function sendGuideAssignmentEmail(assignmentData) {
  try {
    ...
    if (!guideEmail || !guideEmail.trim() || !guideEmail.includes("@")) {
      ...
      return {
        success: false,
        error: "Invalid or missing guide email address",
      };
    }
    ...
    if (response.success) {
      console.log(
        `Guide assignment email sent to ${guideEmail} for booking ${bookingId}`
      );
      return { success: true, messageId: response.id };
    } else {
      console.error(
        "Failed to send guide assignment email. Response:",
        JSON.stringify(response, null, 2)
      );
      return {
        success: false,
